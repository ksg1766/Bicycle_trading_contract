...
/*
    //자전거의 가격을 담을 맵핑(이름:가격)
    mapping(string => uint256) private BikePrice;
    //자전거 이름과 사진 맵핑(이름:사진)
    mapping(string => bytes) private BikeImage;
    //자전거와 판매자 맵핑(이름:주소)
    mapping(string => address) private BikeSeller;
    //자전거와 수수료(이름:수수료)
    mapping(string => uint256) private BikeFee;
*/    
    //자전거 구매 시간(이름:시간)
    ...
    //구매확정기한 지났는지 체크
    function ifTooLate(uint _time) public view returns(bool){
    ...
    //자전거 삭제(자전거 구매 확정 시 사용 할 예정) - 필요가 있는지 모르겠음
    ...
    function buyBike(string memory _BikeName) public payable {
        ...
        (bool __success,) = address(this).call{value:BikeInfo[_BikeName].BikePrice / 100}("");
        require(_success,"Failed to pay");
        require(__success,"Failed to pay");
    ...
    function confirmOrRefundPurchase(string memory _BikeName) public payable {
        ...
        else { //3일 지났으면 그냥 돈돈 돌려주기. 다만 구매자 확인을 거쳐야 함.
            refund(_BikeName);
            BikeNameArray.push(_BikeName);
            //구매자 명단도 삭제제
            delete BikeBuyer[_BikeName];
    ...
    //환불
    function refund(string memory _BikeName) public payable {
        require(BikeBuyer[_BikeName] == msg.sender, "This account hasn't purchased the product");
    ...
    function zzzcontractKiller() public Owned {
